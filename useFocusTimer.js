...
// Shared across every tab/window on this origin (unlike everything else in
// this file, which is per-hook-instance state). Used purely as a cross-tab
// "has this exact session already been logged" marker — see finish() below.
const LOGGED_SESSION_KEY = "sb.focusTimer.loggedSessionId.v1";
...
export function useFocusTimer({ onComplete } = {}) {
  ...
  // Identifies one specific run of the timer (assigned in start(), cleared
  // on reset). Persisted so that if this same countdown is also open in
  // another tab/window, both tabs agree on the id for "this session" —
  // that's what lets finish() below dedupe a completion that multiple tabs
  ...
  useEffect(() => {
    savePersisted({
      ...
      secondsLeft, endAt: endAtRef.current, sessionInProgress, sessionId,
    });
  }, [modeMinutes, mode, running, askDone, soundOn, radioChoice, radioCustomUrl, startedMinutes, secondsLeft, sessionInProgress, sessionId]);
  ...
  const finish = useCallback(() => {
    ...
    // Cross-tab dedupe: finishedRef only stops this one tab's hook instance
    // from logging twice. If the same countdown is also open in another
    // tab/window (e.g. a PWA reopened from recents, or a second browser
    // tab), each one loads its own copy of the persisted state and runs its
    // own interval, so each independently crosses secondsLeft <= 0 within
    // moments of the others and would otherwise insert its own duplicate
    // timer_sessions row. localStorage IS shared across tabs on the same
    // origin, so use it as a "first tab past the post wins" marker keyed by
    // this session's id — every other tab checking the same id sees it's
    // already claimed and skips the insert.
    let alreadyLoggedElsewhere = false;
    if (sessionId) {
      try {
        alreadyLoggedElsewhere = localStorage.getItem(LOGGED_SESSION_KEY) === sessionId;
        if (!alreadyLoggedElsewhere) localStorage.setItem(LOGGED_SESSION_KEY, sessionId);
      } catch {
        /* storage unavailable — falls back to the per-tab finishedRef guard only */
      }
    }
  ...
  }, [soundOn, mode, modeMinutes, startedMinutes, sessionId]);
