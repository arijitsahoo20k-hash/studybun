...
export default function App() {
  ...
  const addSession = async (payload) => {
    await sessionsQ.insert({ session_date: todayStr(), ...payload });
    if (payload.subject && payload.chapter) {
      const cur = getChStatus(`${payload.subject}::${payload.chapter}`);
      if (cur.status === "Not Started") await chapters.upsert(payload.subject, payload.chapter, { status: "Studying" });
    }
  ...
  const completeChapter = async (c) => {
    ...
    await revisionsQ.insert({ subject: c.subject, chapter: c.name, due_date: due, revision_number: 1, status: "Pending" });
    fireCelebrate();
    showToast(`${c.name} completed! First revision scheduled 🌸`);
  };

  const addQuestions = async (payload) => { await questionsQ.insert({ log_date: todayStr(), ...payload }); showToast(`+${payload.count} questions logged 🥕`); };
  const addMock = async (m) => { await mocksQ.insert({ mock_date: todayStr(), ...m }); fireCelebrate(); showToast("Mock added — great effort showing up for it."); };
  const completeRevision = async (id) => { await revisionsQ.update(id, { status: "Completed" }); fireCelebrate(); showToast("Revision complete — that chapter just got stickier 🌷"); };
  const addTask = (t) => tasksQ.insert({ due_date: todayStr(), status: "Pending", ...t });
  const toggleTask = (t) => tasksQ.update(t.id, { status: t.status === "Pending" ? "Completed" : "Pending" });
  const deleteTask = (id) => tasksQ.remove(id);
  ...
  const pageProps = {
    ...
    questions, addQuestions, mocks, addMock, revisions, completeRevision,
  ...
  return (
    <div className="sb-app" style={cssVars} data-stitched={theme.stitched ? "true" : "false"}>
      ...
      {toast && <div className="sb-toast"><Mascot species={mascot} mood="celebrate" size={28} hop={hopping} /> <span className="sb-quote">{toast}</span></div>}
