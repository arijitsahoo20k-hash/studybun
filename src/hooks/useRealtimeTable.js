...
/**
 * Keeps a Supabase table's rows (scoped to this device) in sync in real time.
 * Falls back to local in-memory state if Supabase isn't reachable, so the UI
 * never hard-crashes just because env vars aren't set up yet.
 */
export function useRealtimeTable(table, { orderBy = "created_at", ascending = false } = {}) {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      .channel(`rt:${table}:${deviceId}`)
      .on(
        "postgres_changes",
        { event: "*", schema: "public", table, filter: `device_id=eq.${deviceId}` },
        (payload) => {
          setRows((prev) => {
...
/** Single-row-per-device table (profiles, user_settings, user_statistics). */
export function useDeviceRow(table, defaults = {}) {
  ...
  useEffect(() => {
    let active = true;
    async function load() {
      const { data, error } = await supabase.from(table).select("*").eq("device_id", deviceId).maybeSingle();
      if (!active) return;
      if (!data && !error) {
