...
function Eye({ state, mirror }) {
  ...
  if (state === "arc") return <path d={`M ${-4.4 * flip} 0.6 Q 0 ${-4.2} ${4.4 * flip} 0.6`} stroke="var(--figure-ink)" strokeWidth="2.2" fill="none" strokeLinecap="round" />;
  if (state === "line") return <path d={`M ${-4 * flip} 0 Q 0 1 ${4 * flip} 0`} stroke="var(--figure-ink)" strokeWidth="2" fill="none" strokeLinecap="round" />;
  if (state === "worry") return <path d={`M ${-3.6 * flip} 1 Q 0 -2 ${3.6 * flip} 1`} stroke="var(--figure-ink)" strokeWidth="2.1" fill="none" strokeLinecap="round" />;
  return (
    <>
      <circle cx="0" cy="0" r="3.4" fill="var(--figure-ink)" />
...
export default function Bear({ mood = "idle", size = 72, hop = false, peek = false }) {
  ...
  return (
    <svg width={size} height={size} viewBox="-42 -50 84 92" style={{ overflow: "visible", flexShrink: 0 }}
      ...
      <ellipse
        cx="-24" cy="30" rx="9" ry="7" fill="var(--figure-body)" {...OUTLINE}
      ...
      <circle cx="-22" cy="-23" r="5" fill="var(--figure-inner)" />
      <circle cx="22" cy="-23" r="5" fill="var(--figure-inner)" />

      {/* big, wide, chubby head -- noticeably larger than every other species */}
      <ellipse cx="0" cy="0" rx="29" ry="27" fill="var(--figure-body)" stroke="var(--figure-outline)" strokeWidth="2.4" />

      {/* big rounded muzzle patch, wider and lower than any other species' snout */}
      <ellipse cx="0" cy="15" rx="17" ry="12" fill="var(--figure-inner)" stroke="var(--figure-outline)" strokeWidth="1.4" />
      <ellipse cx="0" cy="9" rx="4.4" ry="3" fill="var(--figure-ink)" />
      ...
      <g transform="translate(-11,-9)">
        ...
        {m.brow && <path d="M -6 -6 Q -1 -9 3 -6.5" stroke="var(--figure-ink)" strokeWidth="1.5" fill="none" strokeLinecap="round" />}
      </g>
      <g transform="translate(11,-9)">
      ...
      <path d={m.mouth} stroke="var(--figure-ink)" strokeWidth="2" fill="none" strokeLinecap="round" />

      {m.honey && (
        <g transform="translate(-33,16)">
          <path d="M -6 0 L 6 0 L 5 12 Q 0 15 -5 12 Z" fill="#E8A33D" stroke="var(--figure-outline)" strokeWidth="1.4" strokeLinejoin="round" />
          <rect x="-7" y="-3" width="14" height="4" rx="1.5" fill="var(--figure-outline)" />
        </g>
      )}
      {m.paw && <ellipse cx="14" cy="8" rx="5" ry="4.5" fill="var(--figure-inner)" stroke="var(--figure-outline)" strokeWidth="1.4" transform="rotate(20 14 8)" />}
