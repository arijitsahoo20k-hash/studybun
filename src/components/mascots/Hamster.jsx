...
function Eye({ state, mirror }) {
  ...
  if (state === "arc") return <path d={`M ${-3 * flip} 0.5 Q 0 ${-2.6} ${3 * flip} 0.5`} stroke="var(--figure-ink)" strokeWidth="1.8" fill="none" strokeLinecap="round" />;
  if (state === "line") return <path d={`M ${-2.6 * flip} 0 Q 0 0.7 ${2.6 * flip} 0`} stroke="var(--figure-ink)" strokeWidth="1.6" fill="none" strokeLinecap="round" />;
  if (state === "worry") return <path d={`M ${-2.4 * flip} 0.8 Q 0 -1.4 ${2.4 * flip} 0.8`} stroke="var(--figure-ink)" strokeWidth="1.7" fill="none" strokeLinecap="round" />;
  const r = state === "wide" ? 3.1 : 2.6;
  return (
    <>
      <circle cx="0" cy="0" r={r} fill="var(--figure-ink)" />
      <circle cx={-0.85 * flip} cy="-0.85" r="0.85" fill="var(--figure-body)" />
    </>
  );
}
...
export default function Hamster({ mood = "idle", size = 72, hop = false, peek = false }) {
  ...
  return (
    <svg width={size} height={size} viewBox="-42 -50 84 92" style={{ overflow: "visible", flexShrink: 0 }}
      ...
      <ellipse cx="0" cy="24" rx="4" ry="3" fill="var(--figure-inner)" {...OUTLINE} strokeWidth="1.2" />

      {/* stuffed cheek pouches, size breathes with mood */}
      <ellipse className="sb-hamster-cheek" cx="-28" cy="8" rx={11 * m.cheek} ry={9 * m.cheek} fill="var(--figure-inner)" stroke="var(--figure-outline)" strokeWidth="1.4" />
      <ellipse className="sb-hamster-cheek" cx="28" cy="8" rx={11 * m.cheek} ry={9 * m.cheek} fill="var(--figure-inner)" stroke="var(--figure-outline)" strokeWidth="1.4" />
      ...
      <circle cx="-19" cy="-13" r="3.2" fill="var(--figure-inner)" />
      <circle cx="19" cy="-13" r="3.2" fill="var(--figure-inner)" />

      {/* short, wide, squashed head */}
      <ellipse cx="0" cy="0" rx="30" ry="21" fill="var(--figure-body)" stroke="var(--figure-outline)" strokeWidth="2.4" />
      ...
      <g transform="translate(-8,-3)">
        ...
        {m.brow && <path d="M -4 -4.5 Q -0.5 -6.5 2 -4.5" stroke="var(--figure-ink)" strokeWidth="1.2" fill="none" strokeLinecap="round" />}
      </g>
      <g transform="translate(8,-3)"><Eye state={eyeState} mirror={true} /></g>

      <path d={m.mouth} stroke="var(--figure-ink)" strokeWidth="1.7" fill="none" strokeLinecap="round" />

      <line x1="-3" y1="8" x2="-15" y2="6" stroke="var(--figure-ink)" strokeWidth="0.8" opacity="0.5" />
      <line x1="-3" y1="11" x2="-15" y2="12" stroke="var(--figure-ink)" strokeWidth="0.8" opacity="0.5" />
      <line x1="3" y1="8" x2="15" y2="6" stroke="var(--figure-ink)" strokeWidth="0.8" opacity="0.5" />
      <line x1="3" y1="11" x2="15" y2="12" stroke="var(--figure-ink)" strokeWidth="0.8" opacity="0.5" />

      {/* little buck teeth, unique to the hamster */}
      <rect x="-3" y="8" width="2.6" height="4" rx="0.6" fill="var(--figure-body)" stroke="var(--figure-outline)" strokeWidth="1" />
      <rect x="0.4" y="8" width="2.6" height="4" rx="0.6" fill="var(--figure-body)" stroke="var(--figure-outline)" strokeWidth="1" />

      {m.seed && (
        <g transform="translate(-30,-24) rotate(-20)">
          <path d="M 0 0 Q 4 4 0 10 Q -4 4 0 0 Z" fill="#B8975A" stroke="var(--figure-outline)" strokeWidth="1" />
        </g>
      )}
